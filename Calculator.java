...
public class Calculator {
    ...
    public double[] variance(double[] array){ // creating a method "variance"
        ...
        for(int i=0; i<array.length; i++){
            mean = mean + array[i]; // calculating the sum of the elements in the array
        }

        mean /= array.length;
        for (int i = 0; i < array.length; i++) {
            variance += Math.pow(array[i] - mean, 2); // calculating variance
        }
        variance = variance / array.length;
        result[0] = variance;  // strong variance value in the result
        return result;
    }
    public double[] standarddev(double[] array) { // creating a method standarddev
        ...
        for (double num : array) {
            standardDeviation += Math.pow(num - mean, 2); // calculating standard deviation
        }
